<img width="2000" height="600" alt="coverimage" src="https://github.com/user-attachments/assets/bbcc63eb-d11e-475c-93f2-c14f90552888" />

**👋 Hi there, I'm Anne Merinka!**

**🔐 Cybersecurity Enthusiast | Python, Linux & Secure Web Applications**

**ABOUT ME**<br>
🎓 Computer Security Undergraduate — University of Plymouth (NSBM Green University)<br>
🛡️ Passionate about cybersecurity, secure authentication systems, threat detection, and secure web application development<br>
💻 Experienced with Python, Flask, Linux environments, virtualization, and web technologies through academic and personal projects<br>
🔍 Interested in Security Operations (SOC), authentication security, infrastructure security, and practical cybersecurity solutions<br>
🚀 Currently building technical projects focused on login security, infrastructure automation, and secure system development<br>
🌱 Continuously learning cybersecurity tools, Linux systems, SIEM concepts, and secure application practices<br>

**🔍 Areas of Interest**
-Cybersecurity<br>
-Secure Authentication Systems<br>
-Web Security<br>
-Threat Detection<br>
-Security Operations (SOC)<br>
-Linux & System Administration<br>
-Infrastructure & Network Security<br>
-Python-Based Security Tools<br>
-Secure Application Development<br>

**🚀 What I'm Working On**
🔐 Non-Locking Bot Mitigation System for Web Login Security (Final Year Project)<br>
⚙️ Containerized Infrastructure & SDN-Based Adaptive Streaming Environment<br>
🎬 Cinema Ticketing System using Java, JSP, Servlets & MySQL<br>
🐧 Exploring Linux environments, virtualization, and cybersecurity tools<br>
📚 Learning SIEM concepts, log analysis, and security monitoring fundamentals<br>

**🛠 Tech Stack**<br>

**💻 Programming Languages**<br>
-Python<br>
-Java<br>
-C#<br>
-JavaScript<br>
-HTML<br>
-CSS<br>

**⚙️ Frameworks & Technologies**<br>
-Flask<br>
-J-SP & Servlets<br>
-Docker<br>
-Docker Compose<br>
-Terraform<br>
-Ansible<br>

**🐧 Operating Systems & Platforms**<br>
-Linux (Kali Linux & Ubuntu)<br>
-Windows<br>
-VMware<br>
-Virtual Machines<br>

**🌐 Cybersecurity & Networking**<br>
-Web Security<br>
-Authentication Systems<br>
-Basic Network Security<br>
-Threat Detection Concepts<br>
-Command Line Usage<br>
-Infrastructure Automation<br>
-SDN Concepts<br>

**🗄️ Databases**<br>
-MySQL<br>
-SQLite<br>

**🛠 Tools & Platforms**<br>
-Git<br>
-GitHub<br>
-Visual Studio Code<br>
-Apache Tomcat<br>
-Mininet<br>
-Figma<br>
-Canva<br>
-Microsoft Office<br>

**📌 Featured Projects**<br>
#🔐 Non-Locking Bot Mitigation System for Web Login Security<br>
Developed a web-based authentication security system designed to detect and mitigate automated login bot attacks without locking legitimate users out of the system
Implemented behavioral analysis, risk scoring, honeypot detection, and adaptive security mechanisms<br>
Stack: Python · Flask · HTML · CSS · JavaScript

#⚙️ Containerized Infrastructure & SDN Simulation<br>
Developed and tested a containerized infrastructure and SDN environment using Ubuntu Linux and VMware
Implemented automation using Docker, Terraform, and Ansible while evaluating adaptive streaming performance under varying network conditions<br>
Stack: Docker · Terraform · Ansible · Ubuntu · VMware · Python · Mininet

#🎬 Cinema Ticketing System<br>
Developed a web-based cinema ticket booking platform with collision-free seat reservation functionality
Implemented backend logic and database integration within a collaborative software engineering project<br>
Stack: Java · JSP · Servlets · MySQL · Apache Tomcat

**🌱 Currently Learning**<br>
-SIEM & Security Monitoring<br>
-Threat Detection & Log Analysis<br>
-Linux System Administration<br>
-Cybersecurity Operations<br>
-Cloud & Infrastructure Security<br>
-Docker & Containerization<br>

**🎯 Career Goal**<br>
Aspiring to build a career in cybersecurity and secure system development, with a strong interest in Security Operations (SOC), authentication security, and practical defensive security solutions.

🔗 Connect With Me<br>
💼 LinkedIn: www.linkedin.com/in/anne-merinka<br>
📧 Email: annemerinka.dev@gmail.com<br>
📫 GitHub: https://github.com/annemerinka<br>



